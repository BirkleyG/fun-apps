...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => {
    ...
    const cleanKey = `timeLoopCyoaSwClean_${APP_VERSION}`;
    const cleanup = () => {
      if (window.localStorage.getItem(cleanKey)) return Promise.resolve(false);
      window.localStorage.setItem(cleanKey, "1");
      return navigator.serviceWorker.getRegistrations().then((regs) => {
        const targets = regs.filter((r) => r.scope.includes("/time-loop-cyoa/") && !r.active?.scriptURL?.includes(`sw.js?v=${APP_VERSION}`));
        return Promise.all(targets.map((r) => r.unregister())).then(() => true);
      }).then((didUnregister) => {
        if (!("caches" in window)) return didUnregister;
        return caches.keys().then((keys) => Promise.all(keys.filter((k) => k.startsWith("time-loop-cyoa-")).map((k) => caches.delete(k)))).then(() => didUnregister);
      }).catch(() => false);
    };
